//! Configuration file support for hermes-ssd-llm
//!
//! Loads settings from `hermes-ssd-llm.toml` (or `$SSD_LLM_CONFIG`).
...
impl Config {
    ...
    fn find_config_file() -> Option<PathBuf> {
        let mut candidates: Vec<PathBuf> = vec![PathBuf::from("hermes-ssd-llm.toml")];
        if let Some(dir) = dirs_config() {
            candidates.push(dir.join("hermes-ssd-llm.toml"));
        }
    ...
    pub fn default_toml() -> String {
        r#"# hermes-ssd-llm configuration file
...
fn dirs_config() -> Option<PathBuf> {
    #[cfg(target_os = "macos")]
    {
        std::env::var("HOME")
            .ok()
            .map(|h| PathBuf::from(h).join(".config").join("hermes-ssd-llm"))
    }
    #[cfg(not(target_os = "macos"))]
    {
        std::env::var("XDG_CONFIG_HOME")
            .ok()
            .map(|d| PathBuf::from(d).join("hermes-ssd-llm"))
            .or_else(|| {
                std::env::var("HOME")
                    .ok()
                    .map(|h| PathBuf::from(h).join(".config").join("hermes-ssd-llm"))
